...
def generate_pydantic_models(parsed_data):
    """
    Generate Pydantic input/output models with function definitions for Python, JavaScript, and Go.
    """
    models = {}
...
"""
                ...
                function_code = f"""
function {function['name']}({', '.join(function['parameters'])}) {{
    // {function['docstring']}
    return { { 'status': 'success' } };
}}
"""
                ...
                function_code = f"""
func {function['name']}({', '.join(function['parameters'])}) {output_type} {{
    // {function['docstring']}
    return "Success"
}}
"""
            else:
                function_code = """// Unsupported language"""
...
def save_pydantic_models(models, output_dir=None):
    """Save Pydantic models to models/generated_models/"""
