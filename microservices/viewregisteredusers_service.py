...
from models.generated_models.Viewregisteredusers import ViewregisteredusersInput, ViewregisteredusersOutput
...
@router.post("/viewregisteredusers_service/viewregisteredusers")
def process_viewregisteredusers(data: ViewregisteredusersInput):
    """
    Dynamically execute ViewRegisteredUsers from generated models.
    """
    import json
    import os
    from pathlib import Path
    ...
    model_path = MODELS_DIR / "Viewregisteredusers.py"
    function_to_call = dynamic_import_function(str(model_path), "ViewRegisteredUsers")
    if function_to_call:
        try:
            ...
            expected_fields = ['users', 'count', 'success']
            result = ensure_output_has_required_fields(result, expected_fields)
            
            # Create response object
            response_obj = ViewregisteredusersOutput(**result)

            # Save the result to a JSON file
            save_dir = Path("microservices/saved_outputs")
            save_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
            save_path = save_dir / f"viewregisteredusers_service_viewregisteredusers.json"
            if save_path.exists():
            ...
            raise HTTPException(status_code=500, detail=f"Function 'ViewRegisteredUsers' execution failed: {str(e)}")
    raise HTTPException(status_code=404, detail=f"Function 'ViewRegisteredUsers' not found in generated models")