...
def process_admincnnmodel(data: AdmincnnmodelInput):
    ...
    if function_to_call:
        try:
            ...
            result = function_to_call(input_data.get("model_name"), input_data.get("model_config"), input_data.get("training_data"), input_data.get("epochs"), input_data.get("batch_size"))
            
            # Ensure result has all required fields
            expected_fields = ['status', 'model_id', 'training_time', 'accuracy']
            result = ensure_output_has_required_fields(result, expected_fields)
            
            # Create response object
            response_obj = AdmincnnmodelOutput(**result)

            # Save the result to a JSON file
            save_dir = Path("microservices/saved_outputs")
            save_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
            save_path = save_dir / f"admincnnmodel_service_admincnnmodel.json"
            if save_path.exists():
            ...
            raise HTTPException(status_code=500, detail=f"Function 'adminCNNModel' execution failed: {str(e)}")
    raise HTTPException(status_code=404, detail=f"Function 'adminCNNModel' not found in generated models")