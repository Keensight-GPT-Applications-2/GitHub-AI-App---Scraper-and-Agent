...
@router.post("/adminactivausers_service/adminactivausers")
def process_adminactivausers(data: AdminactivausersInput):
    """
    Dynamically execute AdminActivaUsers from generated models.
    """
    import json
    import os
    from pathlib import Path
    ...
    model_path = MODELS_DIR / "Adminactivausers.py"
    function_to_call = dynamic_import_function(str(model_path), "AdminActivaUsers")
    if function_to_call:
        try:
            ...
            result = function_to_call(input_data.get("user_ids"), input_data.get("is_active"))
            
            # Ensure result has all required fields
            expected_fields = ['success', 'message', 'updated_count']
            result = ensure_output_has_required_fields(result, expected_fields)
            
            # Create response object
            response_obj = AdminactivausersOutput(**result)

            # Save the result to a JSON file
            save_dir = Path("microservices/saved_outputs")
            save_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
            save_path = save_dir / f"adminactivausers_service_adminactivausers.json"
            if save_path.exists():
            ...
            raise HTTPException(status_code=500, detail=f"Function 'AdminActivaUsers' execution failed: {str(e)}")
    raise HTTPException(status_code=404, detail=f"Function 'AdminActivaUsers' not found in generated models")