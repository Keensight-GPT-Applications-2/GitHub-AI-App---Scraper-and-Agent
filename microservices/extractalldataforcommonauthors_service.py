...
def process_extractalldataforcommonauthors(data: ExtractalldataforcommonauthorsInput):
    ...
    if function_to_call:
        print(f"✅ Function extractAllDataForCommonAuthors found in: {model_path}")
        result = function_to_call(**data.dict())  # Pass Pydantic model data as function arguments
        return ExtractalldataforcommonauthorsOutput(result=result)

    raise HTTPException(status_code=404, detail="Function 'extractAllDataForCommonAuthors' not found in generated models")