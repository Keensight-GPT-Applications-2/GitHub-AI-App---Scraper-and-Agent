...
def process_adminlogincheck(data: AdminlogincheckInput):
    ...
    if function_to_call:
        try:
            ...
            result = function_to_call(input_data.get("username"), input_data.get("password"))
            
            # Ensure result has all required fields
            expected_fields = ['status', 'message', 'user_id', 'is_admin']
            result = ensure_output_has_required_fields(result, expected_fields)
            
            # Create response object
            response_obj = AdminlogincheckOutput(**result)

            # Save the result to a JSON file
            save_dir = Path("microservices/saved_outputs")
            save_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
            save_path = save_dir / f"adminlogincheck_service_adminlogincheck.json"
            if save_path.exists():
            ...
            raise HTTPException(status_code=500, detail=f"Function 'AdminLoginCheck' execution failed: {str(e)}")
    raise HTTPException(status_code=404, detail=f"Function 'AdminLoginCheck' not found in generated models")