...
def process_alluserresults(data: AlluserresultsInput):
    ...
    if function_to_call:
        try:
            ...
            response_obj = AlluserresultsOutput(**result)

            # Save the result to a JSON file
            save_dir = Path("microservices/saved_outputs")
            save_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
            save_path = save_dir / f"alluserresults_service_alluserresults.json"
            if save_path.exists():
            ...
            raise HTTPException(status_code=500, detail=f"Function 'AllUserResults' execution failed: {str(e)}")
    raise HTTPException(status_code=404, detail=f"Function 'AllUserResults' not found in generated models")